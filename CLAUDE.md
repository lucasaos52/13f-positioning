...
Everything runs and 63 tests pass, but the pipeline has **only ever been run
against generated fixtures**. Exactly one real EDGAR filing has been through the
parser (`tests/fixtures/real_brk_2005q4_13fhra.txt`), and it exposed a bug that
had made the entire pre-2004 era parse to an empty table in silence.

**The single most valuable thing you can do in this repo is run it against real
data and report what breaks.** Assume more bugs of that kind are waiting.
...
crowdflow datasets --cross-validate 25 -v    # bulk path: ~53 quarterly zips
crowdflow ingest --max-filers 50 -v          # raw path, cheap smoke test first
...
- Only one real filing has been parsed. Expect more layout bugs in the pre-2013
  era; the leading open-source parser reports ~93% success there, so 100% is not
  the target — *loud* failure is.
...
- `datasets.py` has passing structural tests against synthetic zips built with
  the real schema, but has never downloaded an actual quarter. Column names and
  the URL pattern should be verified on first real run.
