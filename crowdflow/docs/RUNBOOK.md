...
## Choose the ingestion path first

Two ways into the same curated store:

- **Bulk (recommended for 2013Q2 onward)** — `crowdflow datasets` downloads
  ~50 DERA zips, curates them, and fetches acceptance timestamps per CIK.
  About an hour end to end for the full panel, most of it the acceptance
  crawl (`--no-acceptance` skips it: every knowledge date then falls back to
  end of filing day, one day conservative, never lookahead).
- **Raw (whole archive, auditable)** — `crowdflow ingest` + `crowdflow
  curate`, one request per filing. A full history is 350-400k filings:
  roughly a day and a half at the rate limit. This is the only path for
  pre-2013 periods, and it is what `--cross-validate` uses to check DERA.

Steps 1-2 below exercise the raw path on a small slice; if you only need
2013Q2+, you can run `crowdflow datasets --cross-validate 25 -v` instead and
skip to step 3.
...
Do not start there. Crawl two years and a handful of filers first:
...
`--max-filers` keeps the filers with the longest sustained 13F-HR records,
which is a longevity bias. It exists to prove the pipeline works. Never report
a number computed under it.
...
Bulk path (2013Q2+, about an hour):

```bash
export CROWDFLOW_USER_AGENT="Your Name your.email@domain.com"
crowdflow datasets --cross-validate 25 -v   # ~50 zips + acceptance crawl -> curated store
crowdflow run --source dera --prices crsp_panel.csv --crosswalk crsp_xwalk.csv -v
```

Raw path (whole archive, a day and more of crawling):
...
Both are resumable. Every response is cached content-addressed on disk, so an
